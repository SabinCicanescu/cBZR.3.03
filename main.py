...
def update_record():
    ...
    # Update record
    try:
        data.updateRecord(namevar.get(), amtvar.get(), dopvar.get(), bamtvar.get(), selected_rowid)
        tv.item(selected, text="", values=(namevar.get(), amtvar.get(), dopvar.get(), bamtvar.get()))
    except Exception as ep:
...
def totalBalance():
    g = data.fetchRecord(query="Select sum(item_price) from expense_record")
    f = data.fetchRecord(query="Select sum(budget_price)-sum(item_price) from expense_record")
    for i in f:
        for j in i:
            messagebox.showinfo('Current Balance: ', f"Balance Remaining: {j} \nTotal Expense: {g}")
...
Label(f1, text='BUDGET VALUE', font=f).grid(row=3, column=0, sticky=W)
...
budget_amt = Entry(f1, font=f, textvariable=bamtvar)
...
budget_amt.grid(row=3, column=1, sticky=EW, padx=(10, 0))
...
# grid placement
cur_date.grid(row=3, column=2, sticky=EW, padx=(10, 0))
